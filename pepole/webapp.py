...
启动：在项目根目录执行
  python -m uvicorn pepole.webapp:app --host 127.0.0.1 --port 8770
或双击 run_web.cmd
"""
...
from pepole.config import WebSearchProvider
from pepole.scenario_overrides import merge_scenario_overrides, override_payload_from_model
...
def _maybe_overlay_deepseek_base_url(body: Any, overlay: dict[str, str | None]) -> None:
    ...
    blob = " ".join(
        ...
        for x in (
            primary,
            fast,
            os.environ.get("PEPOLE_MODEL_PRIMARY"),
            os.environ.get("PEPOLE_MODEL_FAST"),
        )
        if x
    ).lower()
...
def _execute_simulation(
    ...
    from pepole.engine import RunConfig, run_single_simulation
    ...
    with _env_overlay(overlay):
        ...
        primary_spec = body.primary or os.environ.get("PEPOLE_MODEL_PRIMARY") or "openai:gpt-4o"
        fast_spec = body.fast or os.environ.get("PEPOLE_MODEL_FAST") or "openai:gpt-4o-mini"
...
def _execute_resume_simulation(
    ...
    from pepole.config import Scenario
    from pepole.engine import RunConfig, run_single_simulation
    from pepole.pause_store import take_checkpoint
    from pepole.providers.registry import get_client
...
def _execute_plan_evaluate(body: PlanEvaluateRequest) -> dict[str, Any]:
    ...
    from pepole.agents.company_memory import augment_decision_context
    from pepole.agents.risk_milestone import evaluate_internal_desk_feasibility
    from pepole.agents.user_resolution import professionalize_plain_resolution
    from pepole.config import Scenario
    from pepole.context import build_decision_context, enrich_decision_context_for_plan_evaluate
    from pepole.engine import RunConfig, _hydrate_finance, build_initial_state
    from pepole.pause_store import copy_checkpoint
    from pepole.providers.registry import get_client
    from pepole.rules import aggregate_from_cohorts, finalize_tick
    from pepole.state import WorldState
...
def _execute_plan_confirm_execute(body: PlanConfirmExecuteRequest) -> dict[str, Any]:
    ...
    from pepole.pause_store import copy_checkpoint, put_checkpoint
    from pepole.rules import aggregate_from_cohorts, finalize_tick
    from pepole.state import WorldState
...
app = FastAPI(title="pepole", docs_url=None, redoc_url=None)
...
def api_ensemble(body: EnsembleRequest) -> JSONResponse | dict[str, Any]:
    ...
    from pepole.engine import RunConfig, run_ensemble_parallel, summarize_ensemble
    from pepole.providers.registry import get_client
...
def api_run(body: RunRequest) -> JSONResponse | dict[str, Any]:
    from pepole.pause_store import put_checkpoint
