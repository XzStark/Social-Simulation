...
可照常调 DeepSeek 做合成，同时改用 **Brave**（`BRAVE_API_KEY` + `PEPOLE_WEB_SEARCH_PROVIDER=brave`）
...
from pepole.config import RegionalGroundingSpec, Scenario


def _brave_api_key() -> str:
    return (os.environ.get("PEPOLE_BRAVE_API_KEY") or os.environ.get("BRAVE_API_KEY") or "").strip()


def _resolve_provider(rg: RegionalGroundingSpec) -> str:
    env = (os.environ.get("PEPOLE_WEB_SEARCH_PROVIDER") or "").strip().lower()
...
def _search_brave(queries: list[str], max_results: int, api_key: str) -> tuple[list[dict[str, str]], str | None]:
    if not api_key:
        return [], "未设置 BRAVE_API_KEY（或 PEPOLE_BRAVE_API_KEY）。"
    headers = {
